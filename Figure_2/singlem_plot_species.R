...
# Species-Level Taxonomy Barplot - Top 20 Taxa with Normalized Abundances
# Creates stacked barplot with good color contrast + PCoA/NMDS ordination
# Author: Moshe
# Date: 2024
...
cat("Reading species data from", input_file, "...\n")
...
# ===== Calculate mean abundance and select top species =====
species_means <- data_long %>%
  group_by(species) %>%
  summarise(mean_abundance = mean(abundance, na.rm = TRUE)) %>%
  arrange(desc(mean_abundance))

cat("\nTotal species detected:", nrow(species_means), "\n")
...
cat("Showing top", n_top_species, "species, grouping rest as 'Other'\n")

# Group low-abundance species into "Other"
plot_data <- data_long %>%
  mutate(
    species_plot = if_else(species %in% top_species, species, "Other (< top 20)")
  ) %>%
  group_by(treatment, sample, species_plot) %>%
...
# Order species by abundance (with "Other" last)
species_order <- c(
  species_means %>%
...
plot_data$species_plot <- factor(plot_data$species_plot, levels = species_order)

# ===== Generate High-Contrast Color Palette =====
n_colors <- length(unique(plot_data$species_plot)) - 1  # Exclude "Other"
...
colors <- c(
  ...
  "#FB8072",
  # Other major species - varied distinct colors
  "#80B1D3",  # light blue
  "#FDB462",  # light orange
  "#B3DE69",  # olive/tan
  "#BEBADA",  # lavender
...
names(colors) <- levels(plot_data$species_plot)

# ===== Create Plot =====
cat("Creating plot...\n")

p <- ggplot(plot_data, aes(x = replicate, y = abundance, fill = species_plot)) +
  geom_bar(stat = "identity", position = "stack", width = 0.9) +
  facet_grid(~ treatment, scales = "free_x", space = "free_x") +
  scale_fill_manual(values = colors, name = "Species") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 101)) +
  labs(
    title = "Taxonomic Composition - All Samples by Treatment (Top 20 Species)",
    x = "Replicate",
...
# Prepare data for ordination: sample x species matrix
cat("Preparing sample x species matrix...\n")
...
# Create abundance matrix (samples as rows, species as columns)
...
if (file.exists(pcoa_svg)) {
  ...
  cat("Copied PCoA SVG to Final_Figures/Figure_2/Figure_2C_Species_PCoA.svg\n")
}
if (file.exists(pcoa_file)) {
  file.copy(pcoa_file,
            file.path(FINAL_FIG2, "Figure_2C_Species_PCoA.pdf"), overwrite = TRUE)
  cat("Copied PCoA PDF to Final_Figures/Figure_2/Figure_2C_Species_PCoA.pdf\n")
}

# ===== Create species list file =====
species_list_file <- str_replace(output_file, ".pdf", "_species_list.txt")
write_tsv(
  species_means %>%
    mutate(
      rank = row_number(),
      shown_in_plot = if_else(rank <= n_top_species, "Yes", "No (in Other)")
    ),
  species_list_file
)
cat("Complete species list saved to:", species_list_file, "\n")

# ===== Save treatment summary =====
treatment_summary <- plot_data %>%
  filter(species_plot != "Other (< top 20)") %>%
  group_by(treatment, species_plot) %>%
